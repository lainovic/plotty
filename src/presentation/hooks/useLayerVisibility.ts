...
class LayerVisibilityChangeListener implements VisibilityChangeListener {
  ...
  constructor(
    private readonly toggleVisibility: (newVisibility: boolean) => void
  ) {}
...
export function useLayerVisibility<T extends Path<any>>(layer: Layer<T>) {
  ...
  React.useEffect(() => {
    const toggleLayerVisibility = (newVisibility: boolean) => {
      toggleVisibility(layer.id, newVisibility);
    };
    const listener = new LayerVisibilityChangeListener(toggleLayerVisibility);
